...
// svql_macros/src/variant/mod.rs
...
/// Macro for defining variants queries (disjoint variants over sub-netlists).
/// - Note: Commas required between variants (trailing optional).
/// - Generates: Enum `MyEnum<S>` with variants, `WithPath` (delegates to inner), `Variant`,
///   `MatchedVariant`, and `SearchableVariant` (merged context, chained queries).
...
/// ```
///
/// For full usage, see `svql_query/src/variants/and_any.rs` or `dff_any.rs`.
///
/// Limitations: Up to ~10 variants (tuple limits in parallel joins); no connections/validation.
pub fn variant_inner(ts: TokenStream) -> TokenStream {
