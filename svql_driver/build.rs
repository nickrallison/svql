        ...
        quote! {
            ...
            fn #fn_ident() {
                ...
                if let Pattern::Netlist { yosys_module, .. } = tc.pattern {
                    ...
                    let (pattern_key, pattern_design_container) = driver
                        .get_or_load_design(&needle_path, &needle_module, &tc.config.needle_options)
                        .unwrap_or_else(|e| panic!("Failed to load pattern {}: {}", needle_path, e));

                    let (haystack_key, haystack_design_container) = driver
                        .get_or_load_design(&haystack_path, &haystack_module, &tc.config.haystack_options)
                        .unwrap_or_else(|e| panic!("Failed to load haystack {}: {}", haystack_path, e));
                    ...
                    let matches = svql_subgraph::FindSubgraphs::find_subgraphs(
