...
mod tests {
    ...
    fn test_otbn_run_time() {
        ...
        let workspace: PathBuf = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("..");
        // Hold Arcs so the borrows into results live long enough
        let hay_path = workspace.join("examples/fixtures/larger_designs/json/otbn_core.json");
        let hay_arc = driver.get_by_path(&hay_path, "otbn_core").unwrap();
        trace!("Loaded haystack: {}", hay_path.display());

        let and_path = workspace.join("examples/patterns/basic/and/verilog/and_gate.v");
        let and_arc = driver.get_by_path(&and_path, "and_gate").unwrap();
        trace!("Loaded needle: {}", and_path.display());

        // let n1_path = workspace
        //     .join("examples/patterns/security/access_control/locked_reg/json/async_en.json");
        // let n1_arc = driver.get_by_path(&n1_path, "async_en").unwrap();

        // let n2_path = workspace
        //     .join("examples/patterns/security/access_control/locked_reg/json/async_mux.json");
        // let n2_arc = driver.get_by_path(&n2_path, "async_mux").unwrap();

        // let n3_path = workspace
        //     .join("examples/patterns/security/access_control/locked_reg/json/sync_en.json");
        // let n3_arc = driver.get_by_path(&n3_path, "sync_en").unwrap();

        // let n4_path = workspace
        //     .join("examples/patterns/security/access_control/locked_reg/json/sync_mux.json");
        // let n4_arc = driver.get_by_path(&n4_path, "sync_mux").unwrap();

        trace!("Starting timing");
        ...
        trace!("Found {} matches for AND gate", results_and.matches.len());
        ...
        trace!("Test completed in {:?}", time_elapsed);
