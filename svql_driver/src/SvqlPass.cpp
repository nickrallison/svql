...
void svql::print_wire(RTLIL::Wire *wire) {
    ...
    if (wire->is_signed) output += "signed ";
...
SvqlPass::SvqlPass()
    : Pass("svql_driver", "find subcircuits and replace them with cells") {}

void SvqlPass::help() {
    log("\n");
    log("    svql_driver -cmd [-pat <pat_file> <pat_module_name> [options] "
        "[selection]]\n");
    log("    or\n");
    log("    svql_driver -net [port]\n");
    log("\n");
    log("This pass looks for subcircuits that are isomorphic to any of the "
        "modules\n");
    log("in the given map file.\n");
    log("map file can be a Verilog source file (*.v) or an RTLIL source file "
        "(*.il).\n");
    log("\n");
    log("    -pat <pat_file> <pat_module_name>\n");
    log("        use the modules in this file as reference. This option can be "
        "used\n");
    log("        multiple times.\n");
    log("\n");
    log("    -verbose\n");
    ...
    log("\n");
    log("    -constports\n");
    log("        also find instances with constant drivers. this may be "
        "much\n");
    log("        slower than the normal operation.\n");
    log("\n");
    log("    -nodefaultswaps\n");
    log("        normally builtin port swapping rules for internal cells are "
        "used per\n");
    log("        default. This turns that off, so e.g. 'a^b' does not match "
        "'b^a'\n");
    log("        when this option is used.\n");
    log("\n");
    log("    -compat <needle_type> <haystack_type>\n");
    log("        Per default, the cells in the map file (needle) must have "
        "the\n");
    log("        type as the cells in the active design (haystack). This "
        "option\n");
    log("        can be used to register additional pairs of types that "
        "should\n");
    ...
    log("\n");
    log("    -perm <needle_type> <port1>,<port2>[,...] "
        "<portA>,<portB>[,...]\n");
    log("        Register a valid permutation of swappable ports for a "
        "needle\n");
    ...
    log("\n");
    log("This pass does not operate on modules with unprocessed processes in "
        "it.\n");
    log("(I.e. the 'proc' pass should be used first to convert processes to "
        "netlists.)\n");
    log("\n");
    log("This pass can also be used for mining for frequent subcircuits. In "
        "this "
        "mode\n");
    log("the following options are to be used instead of the -map option.\n");
    log("\n");
    log("The modules in the map file may have the attribute 'extract_order' "
        "set "
        "to an\n");
    log("integer value. Then this value is used to determine the order in "
        "which "
        "the pass\n");
    log("tries to map the modules to the design (ascending, default value is "
        "0).\n");
    log("\n");
...
void SvqlPass::execute_cmd(std::vector<std::string> args,
                           RTLIL::Design *design) {
    std::string error_msg;

    // 1. Parse args to config (Rust FFI)
    size_t argsidx;
    std::optional<SvqlRuntimeConfig> cfg =
        parse_args_to_config(argsidx, args, error_msg);
    if (!cfg) {
    ...
    // 4. Run solver
    std::optional<QueryMatchList> match_list =
        run_solver(solver.get(), cfg_ref, needle_design, design, error_msg);
    if (!match_list) {
...
std::optional<SvqlRuntimeConfig> SvqlPass::parse_args_to_config(
    ...
    for (argsidx = 2; argsidx < args.size(); argsidx++) {
        if (args[argsidx] == "-pat" && argsidx + 2 < args.size()) {
            cfg.pat_filename = args[argsidx + 1];
            cfg.pat_module_name = RTLIL::escape_id(args[argsidx + 2]);

            log("Using pattern file `%s` with module `%s`.\n",
                cfg.pat_filename.operator std::string().c_str(),
                cfg.pat_module_name.operator std::string().c_str());

            argsidx += 2;  // Skip the next two arguments
            continue;
        }
        ...
        if (args[argsidx] == "-perm" && argsidx + 3 < args.size()) {
            ...
            for (const auto &port : map_left) perm_port.left.emplace_back(port);
...
std::optional<uint16_t> SvqlPass::parse_args_net(
    size_t &argsidx, const std::vector<std::string> &args,
    std::string &error_msg) {
    uint16_t port = 0;
    ...
    for (argsidx = 2; argsidx < args.size(); argsidx++) {
        if (args[argsidx] == "-port" && argsidx + 1 < args.size()) {
            ...
            try {
                ...
                error_msg = "Invalid port number: " + port_str;
                return std::nullopt;
            } catch (const std::out_of_range &) {
                error_msg = "Port number out of range: " + port_str;
                return std::nullopt;
            }
            continue;
        }
...
std::unique_ptr<SubCircuitReSolver> SvqlPass::create_solver(
    ...
    for (size_t i = 0; i < cfg.compat_pairs.size(); ++i) {
        ...
        solver->addCompatibleTypes(pair.needle.operator std::string(),
                                   pair.haystack.operator std::string());
    }
    ...
    for (size_t i = 0; i < cfg.perm_ports.size(); ++i) {
        ...
        solver->addSwappablePortsPermutation(
            perm.type_name.operator std::string(), map);
    }
    ...
    for (size_t i = 0; i < cfg.ignored_parameters.size(); ++i) {
        ...
        solver->ignoredParams.insert(
            std::make_pair(ip.param_name.operator std::string(),
                           ip.param_value.operator std::string()));
    }
...
RTLIL::Design *SvqlPass::setup_needle_design(const SvqlRuntimeConfig &cfg,
                                             std::string &error_msg) {
    ...
    if (pat_filename.compare(0, 1, "%") == 0) {
        if (!saved_designs.count(pat_filename.substr(1))) {
            error_msg =
                "Saved design `" + pat_filename.substr(1) + "` not found.";
            delete needle_design;
            return nullptr;
        }
        ...
        Frontend::frontend_call(
            needle_design, &f, pat_filename,
            (pat_filename.size() > 3 &&
                     pat_filename.compare(pat_filename.size() - 3,
                                          std::string::npos, ".il") == 0
                 ? "rtlil"
                 : "verilog"));
        f.close();
...
std::optional<QueryMatchList> SvqlPass::run_solver(SubCircuitReSolver *solver,
    ...
    if (needle_design == nullptr) {
        error_msg =
            "Needle design is not set up. Call setup() before running queries.";
        return std::nullopt;
    }

    if (design == nullptr) {
        error_msg =
            "Design is not set. Call execute() with a valid design first.";
        return std::nullopt;
    }
    ...
    for (auto &haystack_it : haystack_map) {
        ...
        solver->solve(results, "needle_" + RTLIL::unescape_id(needle->name),
                      haystack_it.first, false);
    }
    ...
    if (results.size() > 0) {
        for (int i = 0; i < int(results.size()); i++) {
            ...
            for (const auto &it : result.mappings) {
                auto *graphCell =
                    static_cast<RTLIL::Cell *>(it.second.haystackUserData);
                auto *needleCell =
                    static_cast<RTLIL::Cell *>(it.second.needleUserData);

                std::string needle_name =
                    escape_needle_name(needleCell->name.str());
                std::string haystack_name =
                    escape_needle_name(graphCell->name.str());
                ...
                std::vector<RTLIL::Wire *> needle_cell_connections =
                    get_cell_wires(needleCell);
                std::vector<RTLIL::Wire *> haystack_cell_connections =
                    get_cell_wires(graphCell);

                std::vector<std::pair<RTLIL::Wire *, RTLIL::Wire *>>
                    connections;
                for (size_t j = 0;
                     j < std::min(needle_cell_connections.size(),
                                  haystack_cell_connections.size());
                     j++) {
                    connections.emplace_back(needle_cell_connections[j],
                                             haystack_cell_connections[j]);
                }

                for (const auto &pair : connections) {
                    if (needle_ports.find(pair.first->name) !=
                        needle_ports.end()) {
