...
pub struct Mapping<'p, 'd> {
    ...
    needle_to_haystack: HashMap<CellWrapper<'p>, CellWrapper<'d>>,
    /// Design to Pattern cell mapping
    haystack_to_needle: HashMap<CellWrapper<'d>, CellWrapper<'p>>,
}
...
impl<'p, 'd> Mapping<'p, 'd> {
    ...
    pub(super) fn insert(&mut self, needle: CellWrapper<'p>, haystack: CellWrapper<'d>) {
        self.needle_to_haystack
            .insert(needle.clone(), haystack.clone());
        self.haystack_to_needle.insert(haystack, needle);
    }

    pub(super) fn remove_by_needle(&mut self, needle: CellWrapper<'p>) -> Option<CellWrapper<'d>> {
    ...
    pub fn get_haystack_cell(&self, needle: CellWrapper<'p>) -> Option<CellWrapper<'d>> {
        self.needle_to_haystack.get(&needle).cloned()
    }

    // debug ensure that needle & haystack mappings are consistent
    pub fn get_needle_cell(&self, haystack: CellWrapper<'d>) -> Option<CellWrapper<'p>> {
    ...
    pub fn haystack_mapping(&self) -> &HashMap<CellWrapper<'d>, CellWrapper<'p>> {
        &self.haystack_to_needle
    }

    pub fn needle_mapping(&self) -> &HashMap<CellWrapper<'p>, CellWrapper<'d>> {
        &self.needle_to_haystack
    }
