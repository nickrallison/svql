...
- [ ] **Simplify Executor / Driver**
    ...
    - *Goal:* 
        - Ideally query execution could happen inside the driver, and the macro just calls the driver and returns the results
            - It is would have to do something that looks like this: 
```
fn query<Query>(q: Query<Search>) -> Vec<Query<Result>> {
    todo!()
}
```
            - This is not valid rust syntax, but semantically this is what we want
- [ ] **Simplify Macros UI**
...
- [ ] **Memory Organization**
    ...
    - *Goal:* Investigate if there are any good 
