        ...
        Err(e) => {
            ...
            match e {
                SvqlPatError::FileNotFound { .. } => {
                    eprintln!(
                        "💡 Tip: Make sure the Verilog file path is correct and the file exists."
                    );
                }
                SvqlPatError::ModuleNotFound { .. } => {
                    eprintln!(
                        "💡 Tip: Check that the module name is correct and matches a module in the file."
                    );
                }
                SvqlPatError::SyntaxError { .. } => {
                    eprintln!(
                        "💡 Tip: Fix the syntax errors in the Verilog file before extracting patterns."
                    );
                }
                SvqlPatError::YosysExecutionError { .. } => {
                    eprintln!(
                        "💡 Tip: Make sure yosys is installed and accessible, and the plugin library is built."
                    );
                }
                _ => {
