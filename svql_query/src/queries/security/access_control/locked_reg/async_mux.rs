...
pub struct AsyncMuxLockedReg<S>
where
    S: State,
{
    pub path: Instance,
...
impl<S> WithPath<S> for AsyncMuxLockedReg<S>
where
    S: State,
{
    ...
    crate::impl_find_port!(AsyncMuxLockedReg, data_in, clk, resetn, write_en, data_out);
...
impl<S> Netlist<S> for AsyncMuxLockedReg<S>
where
    S: State,
{
    const MODULE_NAME: &'static str = "async_mux";
    const FILE_PATH: &'static str = "examples/patterns/security/access_control/locked_reg/rtlil/async_mux.il";
    const YOSYS: &'static str = "./yosys/yosys";
    const SVQL_DRIVER_PLUGIN: &'static str = "./build/svql_driver/libsvql_driver.so";
}

impl SearchableNetlist for AsyncMuxLockedReg<Search> {
    type Hit = AsyncMuxLockedReg<Match>;
...
mod tests {
    ...
    fn test_async_mux_locked_reg_netlist() {
        ...
        assert_eq!(
            ...
            1,
            "Expected 1 match for AsyncMuxLockedReg, got {}",
            async_mux_search_result.len()
        );
    }
}
