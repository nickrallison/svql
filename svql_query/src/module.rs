...
pub(crate) use crate::module_raw::ModuleRaw;
...
pub trait Module: ModuleRaw {
    type Result;

    /// convert one SanitizedQueryMatch into *your* domain object
    fn from_match(
        &self,
        m: SanitizedQueryMatch,
    ) -> Result<Self::Result, SvqlQueryError>;

    /// default blanket impl – can be overridden if necessary
    fn query_net(
        &self,
        addr: &str,
    ) -> Result<Vec<Self::Result>, SvqlQueryError> {
        let raw = self.query_raw(addr)?;
        raw.into_iter()
            .map(|m| self.from_match(m))
            .collect()
    }
}
...
//
//     fn submodules(&self)  -> Vec<Module> { Vec::new() }
