...
//         out
//     }
//     pub fn query_in_ctx<'ctx>(
//         sdffe_ctx: &'ctx svql_driver::query_ctx::QueryCtx,
//         and_ctx: &'ctx svql_driver::query_ctx::QueryCtx,
//         path: Instance,
//         config: &Config,
//     ) -> Vec<DffThenAnd<svql_query::Match<'ctx, 'ctx>>> {
//         use svql_query::queries::netlist::basic::and::and_gate::AndGate;
//         use svql_query::queries::netlist::basic::dff::Sdffe;

//         // Run sub-queries under their contexts to keep lifetimes stable
//         let sdffe_hits =
//             Sdffe::<Search>::query_in_ctx(sdffe_ctx, path.child("sdffe".to_string()), config);
//         let and_hits =
//             AndGate::<Search>::query_in_ctx(and_ctx, path.child("andg".to_string()), config);

//         let mut out: Vec<DffThenAnd<svql_query::Match<'ctx, 'ctx>>> = Vec::new();

//         for s in sdffe_hits.into_iter() {
//             for a in and_hits.clone().into_iter() {
//                 let cand = DffThenAnd::<svql_query::Match> {
//                     path: path.clone(),
//                     sdffe: s.clone(),
//                     andg: a.clone(),
//                 };
...
//     // let each netlist open its own context using trait methods
//     let sdffe_ctx = Sdffe::<Search>::open_ctx(&driver, &hay_key)?;
//     let and_ctx = AndGate::<Search>::open_ctx(&driver, &hay_key)?;

//     // now query in ctx
//     let config = Config::builder().exact_length().none().build();
//     let root = Instance::root("dff_then_and".to_string());
//     let hits = DffThenAnd::<Search>::query_in_ctx(&sdffe_ctx, &and_ctx, root, &config);

//     log::trace!("main: DffThenAnd matches={}", hits.len());
//     assert_eq!(hits.len(), 2, "expected 2 DffThenAnd matches");

//     // Extra validation/printing retained.
