...
pub trait MatchedComposite<'ctx>: Composite<Match<'ctx>> {
    ...
    fn validate_connection(&self, connection: Connection<Match<'ctx>>) -> bool {
        ...
        tracing::event!(tracing::Level::TRACE, "Found from port: {:?}", from_wire.is_some());
        tracing::event!(tracing::Level::TRACE, "Found to port: {:?}", to_wire.is_some());
        ...
        match (from_wire, to_wire) {
            (Some(from), Some(to)) => {
                ...
                tracing::event!(tracing::Level::TRACE, "From match present: {:?}", from_match.is_some());
                tracing::event!(tracing::Level::TRACE, "To match present: {:?}", to_match.is_some());
                ...
                match (from_match, to_match) {
                    (Some(from_val), Some(to_val)) => {
                        ...
                        tracing::event!(tracing::Level::TRACE, "From cell present: {:?}", from_cell.is_some());
                        tracing::event!(tracing::Level::TRACE, "To cell present: {:?}", to_cell.is_some());
                        ...
                        if let (Some(from_c), Some(to_c)) = (from_cell, to_cell) {
                            ...
                            tracing::event!(tracing::Level::TRACE, "Connection validation failed - missing cell references");
                            false
                        }
                    }
                    _ => {
                        tracing::event!(tracing::Level::TRACE, "Connection validation failed - missing match values");
                        false
                    }
                }
            }
            _ => {
                tracing::event!(tracing::Level::TRACE, "Connection validation failed - could not find ports");
                false
            }
        }
    }
    ...
    fn validate_connections(&self, connections: Vec<Vec<Connection<Match<'ctx>>>>) -> bool {
        ...
        for (i, connection_set) in connections.iter().enumerate() {
            ...
            if !valid {
                tracing::event!(tracing::Level::TRACE, "Connection set {} failed validation", i);
                return false;
            }
        }
        tracing::event!(tracing::Level::TRACE, "All connection sets passed validation");
        true
    }
}
