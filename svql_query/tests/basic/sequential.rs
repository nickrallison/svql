...
// Tests the SdffeThenAnd composite defined in svql_query/src/composites/dff_then_and.rs
query_test!(
    name: test_dff_then_and_composite,
    query: SdffeThenAnd<Search>,
    haystack: ("examples/fixtures/basic/ff/verilog/and_q_double_sdffe.v", "and_q_double_sdffe"),
    ...
    // Looking at dff_then_and.rs:
    // ctx.connect(Some(&self.sdffe.q), Some(&self.and_gate.a));
    // ctx.connect(Some(&self.sdffe.q), Some(&self.and_gate.b));
    // This requires the SAME DFF to feed both inputs of the AND gate.
    // The fixture does NOT do this (it mixes q1 and q2).
    expect: 0
);
