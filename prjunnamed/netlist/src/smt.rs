...
impl<'a, SMT: SmtEngine> SmtBuilder<'a, SMT> {
    ...
    fn bv_const(
        ...
        suffix: &str,
        width: usize,
    ) -> Result<SMT::BitVec, SMT::Error> {
        self.engine
            .declare_bitvec_const(&format!("{prefix}{index}{suffix}"), width)
    }

    fn bv_bind(&mut self, bv_value: SMT::BitVec, width: usize) -> Result<SMT::BitVec, SMT::Error> {
        let bv_temp = self
            .engine
            .declare_bitvec_const(&format!("t{}", self.temp), width)?;
        self.temp += 1;
        self.engine
            .assert(self.engine.build_bitvec_eq(bv_temp.clone(), bv_value))?;
        Ok(bv_temp)
    }

    fn bv_concat(
        &self,
        elems: impl IntoIterator<IntoIter: DoubleEndedIterator<Item = SMT::BitVec>>,
    ) -> SMT::BitVec {
    ...
    fn bv_is_zero(&self, bv_value: SMT::BitVec, width: usize) -> SMT::Bool {
        self.engine
            .build_bitvec_eq(bv_value, self.bv_lit(Const::zero(width)))
    }

    fn bv_to_bool(&self, bv_value: SMT::BitVec) -> SMT::Bool {
        self.engine
            .build_bitvec_eq(bv_value, self.bv_lit(Trit::One))
    }

    fn bool_to_bv(&self, bool_value: SMT::Bool) -> SMT::BitVec {
        self.engine
            .build_bitvec_ite(bool_value, self.bv_lit(Trit::One), self.bv_lit(Trit::Zero))
    }

    fn tv_lit<'b>(&self, value: impl Into<Cow<'b, Const>>) -> SmtTritVec<SMT> {
        let value = value.into();
        let is_undef = Const::from_iter(value.iter().map(|t| {
            if t == Trit::Undef {
                Trit::One
            } else {
                Trit::Zero
        ...
        let is_one = Const::from_iter(value.iter().map(|t| {
            if t == Trit::One {
                Trit::One
            } else {
                Trit::Zero
        ...
        SmtTritVec {
            x: self.bv_lit(is_undef),
            y: self.bv_lit(is_one),
        }
    }
    ...
    fn tv_const(
        ...
        Ok(SmtTritVec {
            x: self.bv_const(prefix, index, "x", width)?,
            y: self.bv_const(prefix, index, "y", width)?,
        })
    }

    fn tv_bind(
        &mut self,
        tv_value: SmtTritVec<SMT>,
        width: usize,
    ) -> Result<SmtTritVec<SMT>, SMT::Error> {
        let bv_temp_x = self
            .engine
            .declare_bitvec_const(&format!("t{}x", self.temp), width)?;
        let bv_temp_y = self
            .engine
            .declare_bitvec_const(&format!("t{}y", self.temp), width)?;
        self.temp += 1;
        self.engine
            .assert(self.engine.build_bitvec_eq(bv_temp_x.clone(), tv_value.x))?;
        self.engine
            .assert(self.engine.build_bitvec_eq(bv_temp_y.clone(), tv_value.y))?;
        Ok(SmtTritVec {
            x: bv_temp_x,
            y: bv_temp_y,
        })
        ...
        for elem in elems.into_iter().rev() {
            bv_result = Some(match bv_result {
                Some((bv_x_msb, bv_y_msb)) => (
                    self.engine.build_concat(bv_x_msb, elem.x),
                    self.engine.build_concat(bv_y_msb, elem.y),
                ),
                None => (elem.x, elem.y),
            })
        }
        bv_result
            .map(|(x, y)| SmtTritVec { x, y })
            .expect("SMT bit vectors cannot be empty")
    }
    ...
    fn tv_is0(&self, tv: SmtTritVec<SMT>) -> SMT::BitVec {
        self.engine
            .build_bvand(self.engine.build_bvnot(tv.y), self.engine.build_bvnot(tv.x))
    }
    ...
    fn tv_not(&self, tv_a: SmtTritVec<SMT>) -> SmtTritVec<SMT> {
        SmtTritVec {
            y: self.engine.build_bvnot(tv_a.y),
            x: tv_a.x,
        }
    }
    ...
    fn tv_and(
        ...
        Ok(SmtTritVec {
            ...
            x: self.engine.build_bvand(
                ...
                self.engine
                    .build_bvnot(self.engine.build_bvor(bv_arg1_is0, bv_arg2_is0)),
            ),
        })
    }
    ...
    fn tv_or(
        ...
        Ok(SmtTritVec {
            ...
            x: self.engine.build_bvand(
                self.engine.build_bvor(tv_a.x, tv_b.x),
                self.engine
                    .build_bvnot(self.engine.build_bvor(bv_arg1_is1, bv_arg2_is1)),
            ),
        })
    }
    ...
    fn tv_xor(&self, tv_a: SmtTritVec<SMT>, tv_b: SmtTritVec<SMT>) -> SmtTritVec<SMT> {
        ...
        // X X X X
        SmtTritVec {
            y: self.engine.build_bvxor(tv_a.y, tv_b.y),
            x: self.engine.build_bvor(tv_a.x, tv_b.x),
        }
    }
    ...
    fn tv_mux(
        ...
        let (tv_s, tv_a, tv_b) = (
            self.tv_bind(tv_s, 1)?,
            self.tv_bind(tv_a, width)?,
            self.tv_bind(tv_b, width)?,
        );
        ...
        let bv_x_s1 = self
            .engine
            .build_bitvec_ite(bool_s_is1.clone(), tv_a.x, bv_x_sx);
        let bv_x_s0 = self
            .engine
            .build_bitvec_ite(bool_s_is0.clone(), tv_b.x, bv_x_s1);
    ...
    fn net(&self, net: Net) -> Result<SmtTritVec<SMT>, SMT::Error> {
        ...
        match self.design.find_cell(net) {
            Ok((cell_ref, _offset)) if matches!(&*cell_ref.get(), Cell::Dff(_)) => {
                self.past_net(net)
            }
            _ => self.curr_net(net),
        }
    ...
    fn invert_net(
        ...
        tv_net: SmtTritVec<SMT>,
    ) -> Result<SmtTritVec<SMT>, SMT::Error> {
        let active = if control_net.is_positive() {
            Const::ones(1)
        } else {
            Const::zero(1)
        };
        Ok(SmtTritVec {
            y: self
                .engine
                .build_bvcomp(tv_net.y, self.engine.build_bitvec_lit(&active)),
            x: tv_net.x,
        })
    }
    ...
    fn cell(&mut self, output: &Value, cell: &Cell) -> Result<SmtTritVec<SMT>, SMT::Error> {
        let prepare_shift =
            |value: &Value, amount: &Value, stride: u32, signed: bool| -> Result<_, SMT::Error> {
                let width = value
                    .len()
                    .max(amount.len() + stride.max(1).ilog2() as usize + 1);
                let value_ext = if signed {
                    value.sext(width)
                } else {
                    value.zext(width)
                };
                let amount_ext = amount.zext(width);
                let (tv_value_ext, tv_amount_ext) =
                    (self.value(&value_ext)?, self.value(&amount_ext)?);
                let bv_stride = self
                    .engine
                    .build_bitvec_lit(&Const::from_uint(stride as u128, width));
        ...
        let bv_cell = match &*cell {
            ...
            Cell::Mux(s, a, b) => {
                self.tv_mux(self.net(*s)?, self.value(a)?, self.value(b)?, output.len())?
            }
            Cell::Adc(a, b, c) => {
                if a.is_empty() {
                    self.net(*c)?
                    ...
                    SmtTritVec {
                        ...
                        y: self.engine.build_bvadd(
                            ...
                            self.engine
                                .build_concat(self.bv_lit(Const::zero(a.len())), tv_c.y),
                        ),
                    }
                }
            }
            ...
            Cell::Eq(a, b) => {
                ...
                let bv_unequal = self.engine.build_bvand(
                    bv_a_xor_b,
                    self.engine
                        .build_bvnot(self.engine.build_bvor(tv_a.x.clone(), tv_b.x.clone())),
                );
            ...
            Cell::ULt(a, b) => {
                let (tv_a, tv_b) = (self.value(a)?, self.value(b)?);
                SmtTritVec {
                    x: self.bool_to_bv(self.engine.build_not(self.engine.build_and(&[
                        self.bv_is_zero(tv_a.x, a.len()),
                        self.bv_is_zero(tv_b.x, b.len()),
                    ]))),
            ...
            Cell::UShr(value, amount, stride) => {
                ...
                let tv_result = SmtTritVec {
                    x: self.engine.build_bitvec_ite(
                        self.bv_is_zero(tv_amount_ext.x, width),
                        self.engine
                            .build_bvlshr(tv_value_ext.x, bv_amount_mul.clone()),
                        self.engine.build_bitvec_lit(&Const::ones(width)),
                    ),
            ...
            Cell::SShr(value, amount, stride) => {
                let (width, tv_value_ext, tv_amount_ext, bv_amount_mul) =
                    prepare_shift(value, amount, *stride, true)?;
                let tv_result = SmtTritVec {
                    x: self.engine.build_bitvec_ite(
                        self.bv_is_zero(tv_amount_ext.x, width),
                        self.engine
                            .build_bvashr(tv_value_ext.x, bv_amount_mul.clone()),
                        self.engine.build_bitvec_lit(&Const::ones(width)),
                    ),
            ...
            Cell::SDivTrunc(..) => {
                unimplemented!("lowering of sdiv_trunc to SMT-LIB is not implemented")
            }
            Cell::SDivFloor(..) => {
                unimplemented!("lowering of sdiv_floor to SMT-LIB is not implemented")
            }
            Cell::SModTrunc(..) => {
                unimplemented!("lowering of smod_trunc to SMT-LIB is not implemented")
            }
            Cell::SModFloor(..) => {
                unimplemented!("lowering of smod_floor to SMT-LIB is not implemented")
            }
            Cell::Match(MatchCell {
                value,
                enable,
                patterns,
            }) => {
                ...
                for alternates in patterns {
                    ...
                    for pattern in alternates {
                        ...
                        for (index, mask) in pattern
                            .iter()
                            .enumerate()
                            .filter(|(_index, mask)| *mask != Trit::Undef)
                        {
                            let tv_eq = self.tv_not(
                                self.tv_xor(self.net(value[index])?, self.net(mask.into())?),
                            );
            ...
            Cell::DLatch(dlatch) => unimplemented!("latches are not lowered to SMT-LIB yet"),
            Cell::Dff(flip_flop) => {
                ...
                if flip_flop.reset_over_enable {
                    ...
                    data = self.tv_mux(
                        reset,
                        self.tv_lit(&flip_flop.reset_value),
                        data,
                        output.len(),
                    )?;
                } else {
                ...
                let value =
                    self.tv_mux(active_edge, data, self.past_value(&output)?, output.len())?;
                if flip_flop.has_clear() {
                    self.tv_mux(
                        clear,
                        self.tv_lit(&flip_flop.clear_value),
                        value,
                        output.len(),
                    )?
                } else {
            ...
            Cell::Target(_target_cell) => {
                unimplemented!("target cells cannot be lowered to SMT-LIB yet")
            }
    ...
    pub fn add_cell(&mut self, output: &Value, cell: &Cell) -> Result<(), SMT::Error> {
        ...
        self.engine
            .assert(self.tv_eq(self.curr_value(output)?, tv_cell))
    }
    ...
    pub fn replace_cell(
        ...
        self.eqs
            .borrow_mut()
            .push(self.tv_refines(self.curr_value(output)?, tv_new_cell));
        Ok(())
    }
    ...
    pub fn replace_net(&mut self, from_net: Net, to_net: Net) -> Result<(), SMT::Error> {
        self.eqs
            .borrow_mut()
            .push(self.tv_refines(self.curr_net(from_net)?, self.net(to_net)?));
        Ok(())
    }

    pub fn replace_void_net(&mut self, from_net: Net, to_net: Net) -> Result<(), SMT::Error> {
        self.engine
            .assert(self.tv_eq(self.curr_net(from_net)?, self.net(to_net)?))
    }

    pub fn replace_dff_net(&mut self, from_net: Net, to_net: Net) -> Result<(), SMT::Error> {
        // Essentially a single induction step.
        self.engine
            .assert(self.tv_eq(self.past_net(from_net)?, self.past_net(to_net)?))?;
        self.eqs
            .borrow_mut()
            .push(self.tv_refines(self.curr_net(from_net)?, self.curr_net(to_net)?));
        Ok(())
    }
    ...
    pub fn check(&mut self) -> Result<Option<SmtExample>, SMT::Error> {
        ...
        let not_and_eqs = self
            .engine
            .build_not(self.engine.build_and(&self.eqs.borrow()[..]));
